...
contract Trading10 {
    ...
    //prosumer構造体
    struct Prosumer {// SPDX-License-Identifier: GPL-3.0
pragma solidity ^0.8.9;
...
contract Trading10 {
    ...
    //再精算プログラム
    function Recalculate(address applicant, uint256 trueEnergy) public returns(uint256 PenaltyETH, uint256 IndexNumber) {
        uint256 imbalance;
        for(uint256 o = 0; o < consumers.length; ) {
            if((consumers[o].consumer == applicant) && (consumers[o].value >= ContractPrice)) {
                if(trueEnergy < consumers[o].kwh) {
                    imbalance = 100 - ((trueEnergy * 100) / consumers[o].kwh);
                    if(imbalance > 10) {
                        RecalculateCount++;
                        return(imbalance, o);
                    } else {
